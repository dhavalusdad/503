...
const TherapistStatus = () => {
  ...
  const renderTherapistItem = (
    ...
    <div
      ref={therapist.length === index + 1 ? lastTherapistElementRef : undefined}
    ...
    >
      <div className='flex items-center gap-3.5'>
        <Image
          imgPath={
            item?.user?.profile_image ? `${SERVER_URL}${item?.user?.profile_image}` : defaultUserPng
          }
      ...
      >
        <span>{item?.user?.user_settings?.[0]?.is_active ? 'Available' : 'Not Available'}</span>
      </div>
    </div>
  );
    ...
    <div className='bg-white p-5 rounded-20px shadow-cardshadow w-full'>
      ...
      <div className='h-80 overflow-y-auto scroll-disable'>
        {therapist.length === 0 ? (
          ...
          <>
            {therapist.map(renderTherapistItem)}
            {isFetchingNextPage && (
              <div className='flex items-center justify-center py-4'>
                <div className='animate-spin rounded-full h-6 w-6 border-b-2 border-primary'></div>
                <span className='ml-2 text-sm text-primarygray'>Loading more therapists...</span>
              </div>
            )}
          </>
        )}
